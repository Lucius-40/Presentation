...
\centering
\begin{tikzpicture}
  \node[inner sep=0pt] (img) at (0,0) {\includegraphics[width=3.8cm]{images/synthetic_face.png}};
  \onslide<2->{
  \foreach \x in {-1.5,-0.8,-0.1,0.6,1.3} {
    \foreach \y in {-0.8,-0.1,0.5} {
      \fill[cyan, opacity=0.7] (\x,\y) circle (0.025);
    }
  }
  \node[font=\tiny, blue!50!black, align=center] at (0,-1.5) {imperceptible cryptographic pattern\\embedded at generation time};
  }
  \onslide<3->{
  \node[rectangle, rounded corners, draw, thick, fill=green!12, minimum width=3cm, minimum height=1cm, align=center] (verify) at (5.0,0) {\small Verified:\\\footnotesize AI-Generated};
  \draw[-{Latex[length=3mm]}, thick] (2.1,0) -- (3.4,0);
  }
\end{tikzpicture}
