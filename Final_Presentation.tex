...
\centering
\begin{tikzpicture}
  \node[inner sep=0pt] (teamimg) {\includegraphics[width=8cm, height=4.5cm, keepaspectratio]{images/team_ai_photo}};
...
\begin{tikzpicture}[imgcap/.style={font=\small}]
  \node<1->[inner sep=0pt] (img1) at (-3.3,1.3) {\includegraphics[width=4cm, height=2.4cm, keepaspectratio]{images/guess_1}};
  \node<1->[imgcap, below=0.1cm of img1] {Image 1};

  \node<1->[inner sep=0pt] (img2) at (3.3,1.3) {\includegraphics[width=4cm, height=2.4cm, keepaspectratio]{images/guess_2}};
  \node<1->[imgcap, below=0.1cm of img2] {Image 2};

  \node<1->[inner sep=0pt] (img3) at (-3.3,-1.9) {\includegraphics[width=4cm, height=2.4cm, keepaspectratio]{images/guess_3}};
  \node<1->[imgcap, below=0.1cm of img3] {Image 3};

  \node<1->[inner sep=0pt] (img4) at (3.3,-1.9) {\includegraphics[width=4cm, height=2.4cm, keepaspectratio]{images/guess_4}};
...
\begin{tikzpicture}[
  ...
  \node[inner sep=0pt] (src) at (0,0) {\includegraphics[width=3.2cm, height=2.2cm, keepaspectratio]{images/example_source}};
  \node[imgcap, below=0.15cm of src] {Source Image};

  \node[inner sep=0pt] (tgt) at (4.5,0) {\includegraphics[width=3.2cm, height=2.2cm, keepaspectratio]{images/example_target}};
  \node[imgcap, below=0.15cm of tgt] {Target Image};

  \node[inner sep=0pt] (out) at (9,0) {\includegraphics[width=3.2cm, height=2.2cm, keepaspectratio]{images/example_output}};
...
\begin{columns}[c]
  ...
  \centering
  \begin{tikzpicture}
    \node[inner sep=0pt] (photo) at (0,0) {\includegraphics[width=3.4cm]{images/face_photo.png}};
  ...
  \begin{tikzpicture}[
    ...
    \onslide<3->{\node[draw, thick, inner sep=1pt] (frame) {\includegraphics[width=2.1cm]{images/face_photo.png}};}
    \onslide<3->{
...
% cat and mouse 
\begin{frame}{Passive defense is a cat and mouse game}
\centering
\begin{tikzpicture}
  \node[anchor=south west, inner sep=0] (img) at (0,0)
    {\includegraphics[width=0.95\textwidth,height=5.2cm,keepaspectratio]{images/cat.jpg}};

  % Use normalized coordinates (0-1) relative to the image corners
  \begin{scope}[x={(img.south east)}, y={(img.north west)}]
    \node[font=\bfseries\small, fill=orange!20, rounded corners, inner sep=2pt]
      at (0.33,0.62) {Detection Models};
    \node[font=\bfseries\small, fill=gray!20, rounded corners, inner sep=2pt]
      at (0.87,0.18) {Generating Models};
  \end{scope}
\end{tikzpicture}

\vspace{0.3cm}
{\footnotesize As generating models get better at creating realistic fakes, detection models must continuously be upgraded to keep pace.}
\end{frame}
...
\begin{tikzpicture}[
  ...
  % Step 1: Show the first box
  \node<1->[box] (created) at (-4.3,0) {\textbf{Content}\\\textbf{Created}};

  % Step 2: Show the second box and the arrow connecting to it
  \node<2->[box] (cred) at (0,0) {\textbf{Content}\\\textbf{Credentials}\\{\tiny origin \textbullet\ creator \textbullet\ edits}};
  \draw<2->[arrowline] (created) -- node[alabel, above] {signed} (cred);

  % Step 3: Show the third box and the arrow connecting to it
  \node<3->[box] (verify) at (4.3,0) {\textbf{User Can}\\\textbf{Verify}};
  \draw<3->[arrowline] (cred) -- node[alabel, above] {shared} (verify);
\end{tikzpicture}

\vspace{0.9cm}

% Step 4: Show the comparison text
\onslide<4->{
